...
A recovery row reads `<age> ago · <tasks> task(s) · <label>`: the snapshot's age, the number of commands it would launch, and its stored `autosaved` label. `Enter` loads the highlighted snapshot; the status line then reads `loaded recovery snapshot; save to name it`, and `w` does exactly that.
