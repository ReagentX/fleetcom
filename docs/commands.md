...
While attached, `Ctrl-\` returns to the dashboard. Every other key, including `Ctrl-C`, `Ctrl-Z`, and `Ctrl-D`, is forwarded to the child. Function keys, modified navigation (e.g. `Alt+Left`), and standard `Ctrl` combinations are forwarded, and the daemon encodes each against the child's live cursor-key mode: a full-screen child that turns on application-cursor mode gets `SS3` arrows (`ESC O A`), while any modifier forces the `CSI` form. `Ctrl-\` refers to the physical chord; the input handler accepts both `Ctrl-\` and crossterm's `Ctrl-4` representation of that chord.
...
- Two encodings are not yet carried to the child: the kitty keyboard protocol and keypad application mode (numpad). A keypad digit sends its normal value.
